...
/**
 * POST /api/messages/batch — add multiple messages to a session in one call
 * Body: { sessionId, messages: [{ role, content, metadata? }] }
 */
async function handlePostBatch(request: Request): Promise<Response> {
  const db = getDb();
  const r2 = getR2();
...
export const Route = createFileRoute("/api/messages/batch")({
  server: {
    handlers: {
      POST: ({ request }) => handlePostBatch(request),
    },
  },
});
